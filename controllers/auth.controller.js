...
// Validation des données d'entrée
  try {
    ...
    return res.status(400).json({ errors: errors.array() });
     ...
     // Renvoie une erreur si le nom de passe est trop court
     return res.status(400).json({ message: 'Le mot de passe doit contenir au moins 6 caractères' });
   }
    // Vérification de l'email si il existe déjà dans la base de données
    const existingUser = await authModel.findOne({email});
    // Renvoie une erreur si l'email existe déjà 
    if(existingUser) {
        return res.status(400).json({ message: 'Votre email existe deja en base de données. Veuillez en choisir un autre'});
    }
    ...
    // Renvoie une erreur si il y a un problème lors de l'enregistrement de l'utilisateur
...
module.exports.login = async (req, res) => {
   ...
   if (!isPasswordValid) {
    console.log('Mot de passe incorrect'); return res.status(400).json({ message: 'Mot de passe incorrect'});
   }
 ...
 res.status(200).json({ message: 'Connexion réussie', token });
    ...
    console.error('Erreur lors de la connexion : ', error.message); res.status(500).json({ message: 'Erreur lors de la connexion'});
 }

};


// Renvoie une erreur si il y a un problème lors de la connexion avec l'utilisateur 